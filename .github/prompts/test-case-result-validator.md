...
**Output:** You MUST output your final JSON verdict as the sole content of your response. Output ONLY the raw JSON object—no markdown fences, no prose, no explanation, no file writing. 
...
1. You MUST output your verdict as raw JSON only. The output MUST contain all 8 category scores as integers in both `categories_old` and `categories_new`.
...
Read the instruction diff provided inline. Identify:
- **Removed requirements** — Sections, structural rules, or content requirements that existed in the old instruction but were deleted.
- **Added requirements** — New sections or rules in the new instruction.
- **Unchanged requirements** — Requirements present in both versions.
...
1. Read the old output content relevant to this category.
2. Read the new output content relevant to this category.
...
Build the complete JSON object with all scores from Phase 4 and the result from Phase 5. Output it as the sole content of your response. The JSON MUST contain the actual integer scores you computed. Verify: `categories_old` has 8 key-value pairs, `categories_new` has 8 key-value pairs, `result` is set. Output only the raw JSON — no markdown, no explanation.
...
**Self-check before outputting:** If either `categories_old` or `categories_new` has fewer than 8 keys or any value is not an integer 1–5, your output is invalid. Fix it before responding.
